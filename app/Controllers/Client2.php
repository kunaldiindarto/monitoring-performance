...
class Client2 extends BaseController
{
    use ResponseTrait;
    public function __construct()
    {
        $this->oeeModel = new Oee2Model();
        $this->performanceModel = new Performance2Model();
        $this->quality2Model = new Quality2Model();
        $this->stat2Model = new Stat2Model();
        $this->pressureModel = new PressureModel();
        $this->lifeTime2 = new Lifetime2Model();
        ...
        $this->nilaiOee2 = new NilaiOee2Model();

        $this->maintenance2 = new Maintenance2Model();
        $this->kondisi = new Kondisi2Model();
        $this->blink = new Blink2Model();
    ...
    public function loop()
    {
        ...
        $lifeTime = $this->lifeTime2->getLifetime2();
        $minorStop = $this->minorStop2->minorStop();
        $failure = $this->failure2->failure();
        $otherLoss = $this->otherLoss2->otherloss();
        // $otherLossMP = $otherLoss['mp'];
        // $otherLossAngin = $otherLoss['angin'];
        $otherLossTotal = $otherLoss['total'];

        $process = $this->process2->getProcess();
        $quality = $this->quality2Model->findAll();

        $failure2 = $this->failure2->failure2();
        $minorStop2 = $this->minorStop2->minorStop2();
        $kondisi = $this->kondisi->getKondisi();
        $blink = $this->blink->getBlink2();
        ...
        foreach ($otherLossTotal as $row2) {
            array_push($jamOlTotal, $row2['jam']);
            array_push($menitOlTotal, $row2['menit']);
            array_push($detikOlTotal, $row2['detik']);
        }
    ...
    public function postKondisi()
    {
        ...
        $data = [
            'kondisi' => $stat,
            'message' => 'kondisi 2 dimasukan ke DB'
        ];
    ...
    public function postLifetime()
    {
        ...
        if ($this->validation->run($data, 'cekLifetime') == FALSE) {
            ...
            $simpan = $this->lifeTime2->insert($data);
            if ($simpan) {
    ...
    public function postOtherloss()
    {
        ...
        if ($this->validation->run($data, 'cek') == FALSE) {
            ...
            $simpan = $this->otherLoss2->insert($data);
            if ($simpan) {
    ...
    public function postMinorstop()
    {
        ...
        if ($this->validation->run($data, 'cek') == FALSE) {
            ...
            $simpan = $this->minorStop2->insert($data);
            if ($simpan) {
    ...
    public function postFailure()
    {
        ...
        if ($this->validation->run($data, 'cek') == FALSE) {
            ...
            $simpan = $this->failure2->insert($data);
            if ($simpan) {
